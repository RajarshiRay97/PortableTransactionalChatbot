...
export class ChatService {
  ...
  url: string = 'http://localhost:5000/chatbotqa?appname=real-estate-management';
  ...
  getBotAnswer(msg: string, isNormalQuestion: boolean) {
    ...
    // Real-Estate-Management App feature
    // **************
    let bhk: number|undefined;
    bhk = this.grabBHKFromUserMsg(msg);
    // **************
    if(isNormalQuestion) {
      if (bhk){
        botMessage = new Message('bot', this.mediatorService.targetMethod(this.getBotMessage(msg), bhk), 0, true, true);
      }
      else{
        botMessage = new Message('bot', this.mediatorService.targetMethod(this.getBotMessage(msg)), 0, true, true);
      }
    }
  ...
  // Real-Estate-Management App feature
  // **************
  // to grab BHK from user message
  grabBHKFromUserMsg(msg: string): number|undefined{
    if (msg.toLowerCase().includes('bhk')){
      let BHKIndex = msg.toLowerCase().indexOf('bhk');
      let numStr = '';
      for (let i=BHKIndex-1;i>=0;i--){
        if (msg[i] !== ' '){
          numStr = msg[i]+numStr;
        }
        else{
